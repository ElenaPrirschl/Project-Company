...
public class Verwaltung implements VerwaltungInterface {
	...
	static List<Mitarbeiter> mitarbeiter_liste = new ArrayList<Mitarbeiter>(SIZE_MITARB_LIST);

	static List<Abteilung> abteilungs_liste = new ArrayList<Abteilung>(SIZE_ABT_LIST);
	...
	// Einfache Speicherung aller Mitarbeiter namen in ein txt file
	try {
	      Path names = Paths.get(".", "AngestelltenNamen.txt");
	      Files.deleteIfExists(names);
	      Files.createFile(names);
	      System.out.println(" File created:" + names);
	      Writer fileWriter = new FileWriter("AngestelltenNamen.txt");
	      fileWriter.write(FileService.AngestelltenNamen(verwaltung).toString());
	      fileWriter.close();
	      String data = Files.readString(names);
	...
	// binäre Speicherung der Mitarbeiter der größten Abteilung
    try {
    	FileOutputStream writeData = new FileOutputStream("abteilung.ser");
        ObjectOutputStream writeStream = new ObjectOutputStream(writeData);
		writeStream.writeObject(FileService.getBiggestAbt(verwaltung));
		writeStream.close();
			} catch (IOException e) {
   ...
   try {
	   ArrayList<Mitarbeiter> mitarbeiter;
		FileInputStream readData = new FileInputStream("abteilung.ser");
	      ObjectInputStream readStream = new ObjectInputStream(readData);
	      mitarbeiter = (ArrayList<Mitarbeiter>) readStream.readObject();
		readStream.close();
	      System.out.println(mitarbeiter.toString());
   		} catch (Exception e) {
   				e.printStackTrace();}
	
   //binäre Speicherung der sortierten Mitarbeiter Liste
   try {
   	FileOutputStream writeData = new FileOutputStream("mitarbeiter_sorted.ser");
       ObjectOutputStream writeStream = new ObjectOutputStream(writeData);
		writeStream.writeObject(FileService.sortMitarbeiter());
		writeStream.close();
			} catch (IOException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
  ...
  try {
	   ArrayList<Mitarbeiter> mitarbeiter;
		FileInputStream readData = new FileInputStream("mitarbeiter_sorted.ser");
	      ObjectInputStream readStream = new ObjectInputStream(readData);
	      mitarbeiter = (ArrayList<Mitarbeiter>) readStream.readObject();
		readStream.close();
	      System.out.println(mitarbeiter.toString());
...
//			
	...
	private void init() {
		...
		this.mitarbeiter_liste.add(new Angestellter(4,"a7",1003,203)); 
